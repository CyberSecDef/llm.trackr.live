...
| M6 | Realtime + Streaming Pipeline | M5 | 6 days | 🟡 In progress (chunks 1–4 done) | |
...
- [x] Frontend hook: `useRunStream(runId)` (chunk 4b) — subscribes to `private-runs.{runId}` on mount, accumulates events into a list, derives a `status` from terminal events (`idle` → `streaming` on `run.started`, → `complete` on `run.completed`, → `errored` on `run.errored`). Captures `window.Echo` into the effect closure on subscribe so cleanup uses the same instance even if the global gets nulled out (test teardown, hot reload). Discriminated-union `RunEvent` type in `resources/js/types/runs.ts` mirrors each PHP event's `broadcastWith()` exactly — change one, change the other. `disabled` flag is true when Echo isn't initialized; the hook becomes a no-op and the page can show an "echo not configured" notice instead of silently failing. 11 Vitest tests cover the subscription/teardown/accumulation/status-transition contract via a `fakeChannel` mock. **SSE fallback lands in chunk 5** (separate ticket from the hook itself).
...
- [x] Debug page `/runs/{id}/debug` (chunk 4b): `App\Http\Controllers\DebugRunController::show` does the same-user-as-run check (mirrors the `runs.{runId}` channel auth invariant) and returns an Inertia render of `Runs/Debug` with the run's static metadata + the channel name to subscribe against. The TSX page renders the metadata header up top, a "subscribed channel + live status" line, and a chronological JSON event list below — append-only as `useRunStream` delivers events. **`config/inertia.php` added** so the Inertia view-finder knows about `.tsx` page paths (defaults are Vue-only); without this, `assertInertia(...)->component('...')` fails with "page component file does not exist" on every test. 4 Pest tests cover authz (redirect/403/404 + 200 render with prop assertions); 6 Vitest tests cover render + event accumulation + status transitions + Echo-disabled notice.
