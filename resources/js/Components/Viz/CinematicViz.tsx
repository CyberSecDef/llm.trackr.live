...
export default function CinematicViz({
    ...
    // M13 chunk 12: track FPS + degraded mode globally for the viz.
    // The hook drives the state machine (< 18 for 2s → degrade;
    // > 24 for 5s → restore). Components opt in via
    // usePerformanceMode() through the context provider below.
    const performanceMode = useFpsTracker();
    ...
    return (
        <PerformanceModeProvider value={performanceMode}>
                            ...
                            <div className="relative flex-1 overflow-hidden rounded-md border border-border bg-slate-950">
                                ...
                                {idle ? (
                                    ...
                                    >
                                        ...
                                        <p className="max-w-md text-[11px] text-muted-foreground/70 italic">
                                            Scene not yet implemented. Lands in a later M13
                                            sub-chunk.
                                        </p>
                                    </div>
                                )}
